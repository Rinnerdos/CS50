...
function Player:init(map)
    ...
    startingPosX = map.tileWidth * 5
    startingPosY = map.tileHeight * ((map.mapHeight - 2) / 2) - self.height
...
function Player:update(dt)
    ...
    self.currentFrame = self.animation:getCurrentFrame()
    self.x = math.max(0, math.min((map.mapWidth * map.tileWidth) - self.width, self.x + self.dx * dt))
    ...
    if self.y > (map.mapHeight / 2) * map.tileWidth + self.height then
        self.x = startingPosX
        self.y = startingPosY
    end
end
